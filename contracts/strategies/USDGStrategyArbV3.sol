...
// File: contracts/strategies/USDGStrategyArbV3.sol
...
contract USDGStrategyArbV3 is Ownable {
    using SafeMath for uint256;
    ...
    // Would be ideal to purchase more USDG but USDG router doesn't yet have an external function that calculate fees to do accurately
    function checkUSDGClaim(bool inWBNBForExecutor) internal view returns (uint256) {
        USDGProtocol yield = USDGProtocol(USDGProtocol(address(tokenList[0].token)).yieldTrackers(0)); // Get the address to the yield yieldTracker
    ...
    function getUSDGClaim() internal {
        ...
        if(amount > 0){
            // Claim
            USDGProtocol usdg = USDGProtocol(address(tokenList[0].token));
