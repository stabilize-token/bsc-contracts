...
// File: contracts/strategies/StandardStrategyArbV5.sol
...
// Belt was removed as it is having issues and added some additional exchanges

contract StandardStrategyArbV5 is Ownable {
    using SafeMath for uint256;
    ...
    address constant VALUEDEFI_4STABLES = address(0x7569f9adabC99780B7A91B16666Bb985177D1DCa);
    address constant VALUEDEFI_2STABLES = address(0xbf8851fC7724EbAeBEcb857a96be7343AA7b0112);
    ...
    uint256 constant EXCHANGE_COUNT = 11;
    ...
    function setupWithdrawTokens() internal {
        ...
        tokenList.push(
            TokenInfo({
                ...
                beltID: 0,
                acryptoID: 2,
                valueID: [0,0], // Value pool 2 doesn't support DAI
        ...
        tokenList.push(
            TokenInfo({
                ...
                beltID: 1,
                acryptoID: 3,
                valueID: [1,0], // Value pool 2 doesn't support USDC
                ellipsisID: 1,
                smoothyID: 2,
        ...
        if(classicArb == false){
            if(_idOut == WBNB_TOKEN_ID){
                ...
                if(_exchangeNum == 0){
                    ...
                    // We use ValueDefi which uses a derivative of Curve
                    ValueLikeExchange router = ValueLikeExchange(VALUEDEFI_4STABLES);
                    return router.calculateSwap(tokenList[_idIn].valueID[0], tokenList[_idOut].valueID[0], _amount);
                    ...
                    // Pancake exchange version 2
                    PancakeRouter router = PancakeRouter(PANCAKE_ROUTER);
                    address[] memory path;
                    if(_idIn == 0 || _idOut == 0){
                        // Already BUSD or output is BUSD
                        path = new address[](2);
                        path[0] = address(tokenList[_idIn].token);
                        path[1] = address(tokenList[_idOut].token);
                    }else{
                        // Convert to BUSD first
                        path = new address[](3);
                        path[0] = address(tokenList[_idIn].token);
                        path[1] = address(tokenList[0].token);
                    ...
                    return _amount;
                }else if(_exchangeNum == 7){
                    // We use ValueDefi which uses a derivative of Curve
                    // This is solo USDT/BUSD pool
                    if(_idIn == 1 || _idOut == 1 || _idIn == 2 || _idOut == 2){return 0;} // DAI and USDC not supported
                    ValueLikeExchange router = ValueLikeExchange(VALUEDEFI_2STABLES);
                    return router.calculateSwap(tokenList[_idIn].valueID[1], tokenList[_idOut].valueID[1], _amount);
                }else if(_exchangeNum >= 8 && _exchangeNum <= 10){
                    // We use Dopple which uses a derivative of Curve
                    // All these pools are solo USDT/BUSD
                    if(_idIn == 1 || _idOut == 1 || _idIn == 2 || _idOut == 2){return 0;} // DAI and USDC not supported
                    uint256 _doppleID = _exchangeNum.sub(8);
        ...
        if(classicArb == false){
            if(_idOut == WBNB_TOKEN_ID){
                ...
                if(_exchangeNum == 0){
                    ...
                    ValueLikeExchange router = ValueLikeExchange(VALUEDEFI_4STABLES);
                    tokenList[_idIn].token.safeApprove(VALUEDEFI_4STABLES, 0);
                    tokenList[_idIn].token.safeApprove(VALUEDEFI_4STABLES, _amount);
                    router.swap(tokenList[_idIn].valueID[0], tokenList[_idOut].valueID[0], _amount, 1, now.add(60));
                    return;
                }else if(_exchangeNum == 4){
                    // This is for Ellipsis
                    ...
                    ValueLikeExchange router = ValueLikeExchange(VALUEDEFI_2STABLES);
                    tokenList[_idIn].token.safeApprove(VALUEDEFI_2STABLES, 0);
                    tokenList[_idIn].token.safeApprove(VALUEDEFI_2STABLES, _amount);
                    router.swap(tokenList[_idIn].valueID[1], tokenList[_idOut].valueID[1], _amount, 1, now.add(60));
                    return;
                }else if(_exchangeNum >= 8 && _exchangeNum <= 10){
                    // We use Dopple which uses a derivative of Curve
                    // All these pools are solo USDT/BUSD
...

        {
            ...
            if(checkFlash == false){
                ...
                if(buyExchangeNum != sellExchangeNum){
                    ...
                    _normalizedGain = estimateSellAtMaximumProfit(_tokenID, targetID, maxAmountSell.mul(10**tokenList[_tokenID].decimals), sellExchangeNum, true, buyExchangeNum); // This will return our ideal trade size
                    if(_normalizedGain > 0){
                        if(inWBNBForExecutor == true){
                            _normalizedGain = estimateFlashLoanResult(_tokenID, targetID, sellExchangeNum, buyExchangeNum, _normalizedGain, true); // Return profit expected
                        }else{
                            _normalizedGain = estimateFlashLoanResult(_tokenID, targetID, sellExchangeNum, buyExchangeNum, _normalizedGain, false); // Return profit expected
                        }
                    } 
                }                    
            }
