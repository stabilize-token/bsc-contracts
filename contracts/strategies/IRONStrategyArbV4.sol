...
// File: contracts/strategies/IRONStrategyArbV4.sol
...
contract IRONStrategyArbV4 is Ownable {
    using SafeMath for uint256;
    ...
    function expectedProfit(bool inWBNBForExecutor, bool checkFlash) external view returns (uint256,uint256,uint256) {
...

        {
            if(checkFlash == false){
                ...
                _normalizedGain = IRONFlashModule(flashModuleAddress).checkFlashTrade(); // Gain in BUSD
                _allocatedStipend = _allocatedStipend.mul(3);
                targetID = 1;
            }
        }
