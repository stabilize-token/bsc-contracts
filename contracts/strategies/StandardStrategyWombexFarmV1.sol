...
// File: contracts/strategies/StandardStrategyWombexFarmV1.sol
...
contract StandardStrategyWombexFarmV1 is Ownable {
    using SafeMath for uint256;
    ...
    */
    ...
    function checkBestPerformer() internal {
        ...
        for(uint256 i = 0; i < tokenList.length; i++){
            if(i < 2) { continue; }
            WombexRewarderVault vault = WombexRewarderVault(tokenList[i].wombexRewarderVault);
            uint256 poolRate = vault.rewardPerToken(WOM_ADDRESS);
