...
// File: contracts/strategies/USDGStrategyArbV4.sol
...
interface USDGRouter {
    function vault() external view returns (address);
    function swap(address[] calldata, uint256, uint256, address) external;
}

interface USDGToken {
    function yieldTrackers(uint256) external view returns (address);
    function claimable(address) external view returns (uint256);
    function claim(address) external;
}

interface USDGVault {
    function getMaxUsdgAmount() external view returns (uint256);
...
// This will arb across multiple tokens and claim WBNB

contract USDGStrategyArbV4 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public gasPrice = 5e9; // Gas price in wei
    ...
    uint256 public percentRewardDepositors = 90000; // Depositors keep most of the USDG reward
    ...
    address constant GMT_ADDRESS = address(0x99e92123eB77Bc8f999316f622e5222498438784); // BUSD
    address constant PANCAKE_ROUTER_V1 = address(0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F);
    address constant PANCAKE_ROUTER_V2 = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    address constant USDG_ROUTER = address(0xD46B23D042E976F8666F554E928e0Dc7478a8E1f);
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        ...
        // We must use both pancake routers to get transfer amounts
        if(_idIn == 0){
            // First convert to GMT
            TradeRouter cake = TradeRouter(PANCAKE_ROUTER_V2);
            path = new address[](2);
            ...
            // Then BUSD
            cake = TradeRouter(PANCAKE_ROUTER_V1);
            path = new address[](2);
            path[0] = GMT_ADDRESS;
            ...
            if(_idOut == WBNB_TOKEN_ID){
                // Now convert BUSD to WBNB using Pancake 2
                cake = TradeRouter(PANCAKE_ROUTER_V2);
                path = new address[](2);
            ...
            if(_idOut == WBNB_TOKEN_ID){
                ...
                // Convert BUSD to USDG
                TradeRouter cake = TradeRouter(PANCAKE_ROUTER_V1);
                path = new address[](2);
                path[0] = address(tokenList[_idIn].token);
                path[1] = GMT_ADDRESS;
                estimates = cake.getAmountsOut(_amount, path);
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        ...
        if(_idIn == 0){
            ...
            path[0] = address(tokenList[_idIn].token);
            path[1] = GMT_ADDRESS;
            _bal = IERC20(GMT_ADDRESS).balanceOf(address(this));
            tokenList[_idIn].token.safeApprove(PANCAKE_ROUTER_V2, 0);
            tokenList[_idIn].token.safeApprove(PANCAKE_ROUTER_V2, _amount);
            cake.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60));
            _amount = IERC20(GMT_ADDRESS).balanceOf(address(this)).sub(_bal);
            ...
            path[0] = GMT_ADDRESS;
            path[1] = address(tokenList[1].token);
            _bal = tokenList[1].token.balanceOf(address(this));
            IERC20(GMT_ADDRESS).safeApprove(PANCAKE_ROUTER_V1, 0);
            IERC20(GMT_ADDRESS).safeApprove(PANCAKE_ROUTER_V1, _amount);
            cake.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60));
            ...
            if(_idOut == WBNB_TOKEN_ID){
                ...
                path[1] = GMT_ADDRESS;
                _bal = IERC20(GMT_ADDRESS).balanceOf(address(this));
                tokenList[_idIn].token.safeApprove(PANCAKE_ROUTER_V1, 0);
                tokenList[_idIn].token.safeApprove(PANCAKE_ROUTER_V1, _amount);
                cake.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60));
                _amount = IERC20(GMT_ADDRESS).balanceOf(address(this)).sub(_bal); 
                ...
                path[0] = GMT_ADDRESS;
                path[1] = address(tokenList[0].token);
                IERC20(GMT_ADDRESS).safeApprove(PANCAKE_ROUTER_V2, 0);
                IERC20(GMT_ADDRESS).safeApprove(PANCAKE_ROUTER_V2, _amount);
                cake.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60));               
            }
        }    
    }
    ...
    // Unique functions to check claimable WBNB from USDG and disperse it
    // Reward is split between depositors and stakers
    // Convert BNB to USDG to compound gains
    function checkUSDGClaim(bool inWBNBForExecutor) internal view returns (uint256) {
        USDGToken yield = USDGToken(USDGToken(address(tokenList[0].token)).yieldTrackers(0)); // Get the address to the yield yieldTracker
        uint256 amount = yield.claimable(address(this));
        if(amount > 0){
            if(inWBNBForExecutor == false){
                address[] memory path;
                uint256[] memory estimates;
                // Convert it all to BUSD units and return it
                TradeRouter cake = TradeRouter(PANCAKE_ROUTER_V2);
                path = new address[](2);
                path[0] = WBNB_ADDRESS;
                path[1] = address(tokenList[1].token);
                estimates = cake.getAmountsOut(amount, path);
                amount = estimates[estimates.length - 1]; // This is the amount of BUSD, already normalized
            }else{
                // Reduce BNB to percent going to executor + stakers
                amount = amount.mul(DIVISION_FACTOR.sub(percentRewardDepositors)).div(DIVISION_FACTOR);
            }
        }
        return amount;
    }
    
    function getUSDGClaim() internal {
        USDGToken yield = USDGToken(USDGToken(address(tokenList[0].token)).yieldTrackers(0)); // Get the address to the yield yieldTracker
        uint256 amount = yield.claimable(address(this));
        if(amount > 0){
            // Claim
            USDGToken usdg = USDGToken(address(tokenList[0].token));
            uint256 _bal = IERC20(WBNB_ADDRESS).balanceOf(address(this));
            usdg.claim(address(this)); // Pull WBNB to this address
            amount = IERC20(WBNB_ADDRESS).balanceOf(address(this)).sub(_bal);
            if(amount > 0){
                // Convert a certain percent to BUSD
                uint256 _toBUSD = amount.mul(percentRewardDepositors).div(DIVISION_FACTOR);
                address[] memory path;
                if(isUSDGMintFull(_toBUSD) == true){
                    // Convert to BUSD if minter is full
                    TradeRouter cake = TradeRouter(PANCAKE_ROUTER_V2);
                    path = new address[](2);
                    path[0] = WBNB_ADDRESS;
                    path[1] = address(tokenList[1].token);
                    IERC20(WBNB_ADDRESS).safeApprove(PANCAKE_ROUTER_V2, 0);
                    IERC20(WBNB_ADDRESS).safeApprove(PANCAKE_ROUTER_V2, _toBUSD);
                    cake.swapExactTokensForTokens(_toBUSD, 1, path, address(this), now.add(60));
                }else{
                    // Otherwise mint more USDG
                    USDGRouter router = USDGRouter(USDG_ROUTER);
                    path = new address[](2);
                    path[0] = WBNB_ADDRESS;
                    path[1] = address(tokenList[0].token); 
                    IERC20(WBNB_ADDRESS).safeApprove(address(router), 0);
                    IERC20(WBNB_ADDRESS).safeApprove(address(router), _toBUSD);
                    router.swap(path, _toBUSD, 1, address(this));
                }
            }
        }
        // The rest will stay as WBNB to be split by reward pool
    }
    
    /*
    function testDeposit() external payable {
        address[] memory path;
        IWETH(WBNB_ADDRESS).deposit{value: msg.value}();
        uint256 _toBUSD = IERC20(WBNB_ADDRESS).balanceOf(address(this));
        USDGRouter router = USDGRouter(USDG_ROUTER);
        path = new address[](2);
        path[0] = WBNB_ADDRESS;
        path[1] = address(tokenList[0].token); 
    ...
    */
    ...
    function isUSDGMintFull(uint256 _amount) internal view returns (bool) {
        // This will query the vault and determine if the vault cannot mint anymore USDG from WBNB
        USDGVault vault = USDGVault(USDGRouter(USDG_ROUTER).vault());
        if(vault.isSwapEnabled() == false){return true;}
        uint256 usdgBalance = vault.usdgAmounts(WBNB_ADDRESS);
        // Convert amount to USDG
        uint256 priceIn = vault.getMinPrice(WBNB_ADDRESS);
        _amount = _amount.mul(priceIn).div(vault.PRICE_PRECISION());
        usdgBalance = usdgBalance.add(_amount);
        if(usdgBalance > vault.getMaxUsdgAmount()){
            return true; // Cannot mint anymore USDG, keep as BUSD
        }
        return false;
    }
    ...
    function checkAndSwapToken(address _executor, uint256 targetID) internal {
        ...
        // Now handle the rewards, if they are available
        getUSDGClaim();
    ...
    // Returns 1) profit, 2) uint256 tokenID to buy, 3) blocknumber of profit
    function expectedProfit(bool inWBNBForExecutor) external view returns (uint256,uint256,uint256) {
        ...
        if(inWBNBForExecutor == false){
            _normalizedGain = _normalizedGain.add(checkUSDGClaim(false)); // This will be as BUSD total gain
        }
        ...
        if(inWBNBForExecutor == false){
            ...
            uint256 estimate = checkUSDGClaim(true); // This will allow executors to make trades even when no profit available via trade
            if(_normalizedGain == 0 && estimate == 0){
            ...
            if(_normalizedGain > 0){
                ...
                sellBalance = sellBalance.mul(DIVISION_FACTOR.sub(percentDepositor)).div(DIVISION_FACTOR);
                // Estimate output
                estimate = estimate.add(simulateExchange(targetID, WBNB_TOKEN_ID, sellBalance));           
            }
    ...
    // Accepts 1) address for where executor profit should go, 2) uint256 minimum seconds required since last trade, 3) last block before trade expires, 4) uint256 tokenID to buy
    ...
    function startChangeStrategyAllocations(uint256 _pDepositors, uint256 _pRDepositors, uint256 _pStakers, uint256 _pExecutor) external onlyGovernance {
        // Changes strategy allocations in one call
        require(_pDepositors <= 100000 && _pExecutor <= 100000 && _pStakers <= 100000 && _pRDepositors <= 100000,"Percent cannot be greater than 100%");
        _timelockStart = now;
        _timelockType = 4;
        _timelock_data[0] = _pDepositors;
        _timelock_data[1] = _pExecutor;
        _timelock_data[2] = _pStakers;
        _timelock_data[3] = _pRDepositors;
    }
    ...
    function finishChangeStrategyAllocations() external onlyGovernance timelockConditionsMet(4) {
        ...
        percentRewardDepositors = _timelock_data[3];
    }
    // --------------------
