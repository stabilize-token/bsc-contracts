...
// File: contracts/strategies/StandardStrategyArbV8.sol
...
contract StandardStrategyArbV8 is Ownable {
    using SafeMath for uint256;
    ...
    function getDopplePoolAddress(uint256 _id) internal pure returns (address) {
        address[4] memory dopplePools;
        dopplePools[0] = address(0x2EADe35C49f3f1E041576aCE336f5A58C0Ad8968); // Only BUSD and USDC
        dopplePools[1] = address(0xbc42FaDcC37994C65A559Fb7803ed60D90994e9f); // Only BUSD and USDC
        dopplePools[2] = address(0x36E04B29169313D93a056289109ba8A8291e69Ab); // Only BUSD and USDC
        dopplePools[3] = address(0xF8AF8659A2AF27d65bb3e705F0E97b321886031d); // Only BUSD and USDT
        return dopplePools[_id];
    }
    
    // Exchange information
    uint256 constant EXCHANGE_COUNT = 10;
    ...
    function setupWithdrawTokens() internal {
        ...
        tokenList.push(
            TokenInfo({
                ...
                doppleID: [0,0,0,0] // DAI not on Dopple
            })
        );
        ...
        tokenList.push(
            TokenInfo({
                ...
                acryptoID: 3,
                ellipsisID: 1,
                doppleID: [0,2,2,0] // USDC is not in 4th dopple pool
            })
        );
    ...
    */
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount, uint256 _exchangeNum) internal view returns (uint256) {
        if(_idOut == WBNB_TOKEN_ID){
            ...
            }else if(_exchangeNum >= 5 && _exchangeNum <= 8){
                // We use Dopple which uses a derivative of Curve
                // All these pools are solo USDT/BUSD
                if(_idIn == 1 || _idOut == 1){return 0;} // DAI not supported
                if(_exchangeNum == 8 && (_idIn == 2 || _idOut == 2)){return 0;} // USDC not supported for dopple 4th pool
                if(_exchangeNum != 8 && (_idIn == 3 || _idOut == 3)){return 0;} // USDT not supported 
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount, uint256 _exchangeNum) internal {
        if(_idOut == WBNB_TOKEN_ID){
            ...
            if(_exchangeNum == 0){
                ...
                // We use Dopple which uses a derivative of Curve
                // All these pools are solo USDT/BUSD
                if(_idIn == 1 || _idOut == 1){return;} // DAI not supported
                if(_exchangeNum == 8 && (_idIn == 2 || _idOut == 2)){return;} // USDC not supported for dopple 4th pool
                if(_exchangeNum != 8 && (_idIn == 3 || _idOut == 3)){return;} // USDT not supported 
                uint256 _doppleID = _exchangeNum.sub(5);
                address doppleAddress = getDopplePoolAddress(_doppleID);
                ValueLikeExchange router = ValueLikeExchange(doppleAddress);
