...
// File: contracts/strategies/StandardStrategyArbV1.sol
...
contract StandardStrategyArbV1 is Ownable {
    using SafeMath for uint256;
    ...
    // Accepts 1) bool whether to show us executor profit, 2) uint256 token ID to sell
    ...
    function expectedProfit(bool inWBNBForExecutor, uint256 _tokenID) external view returns (uint256,uint256,uint256) {
    ...
    // 3) uint256 tokenID for token we want to sell, 4) uint256 tokenID of the token we want to buy, 5) exchange that we want to use to sell
