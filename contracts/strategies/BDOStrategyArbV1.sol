...
// File: contracts/strategies/BDOStrategyArbV1.sol
...
contract BDOStrategyArbV1 is Ownable {
    using SafeMath for uint256;
    ...
    function setupWithdrawTokens() internal {
        // Start with BDO
        IERC20 _token = IERC20(address(0x190b589cf9Fb8DDEabBFeae36a813FFb2A702454));
        tokenList.push(
            TokenInfo({
                token: _token,
        ...
        // VAI
        _token = IERC20(address(0x4BD17003473389A42DAF6a0a729f6Fdb328BbBd7));
        tokenList.push(
            TokenInfo({
                token: _token,
                decimals: _token.decimals()
    ...
    function expectedProfit(bool inWBNBForExecutor) external view returns (uint256,uint256) {
        ...
        uint256 targetID;
        uint256 minGainNeeded = balance().mul(minPercentProfit).div(DIVISION_FACTOR); // Strat must produce enough gain to run
