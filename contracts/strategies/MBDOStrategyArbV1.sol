...
// File: contracts/strategies/MBDOStrategyArbV1.sol
...
contract MBDOStrategyArbV1 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxAmountSell = 500000; // The maximum amount of tokens that can be sold at once (normalized)
    ...
    uint256 public minAmountProfit = 1e18; // Total profit must be greater than 0.005% for executor to earn from trade
    uint256 public minTradeSplit = 20000; // Minimum amount required before selling a percent of tokens (normalized)
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        TradeRouter pan = TradeRouter(PANCAKE_ROUTER_V1);
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            ...
            // Paths take the tokens between BUSD
            path = new address[](3);
            path[0] = address(tokenList[_idIn].token);
            path[1] = BUSD_ADDRESS;
            path[2] = address(tokenList[_idOut].token);
    ...
    // Returns 1) profit, 2) uint256 tokenID to buy, 3) blocknum of profit
