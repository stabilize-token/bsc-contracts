...
// File: contracts/strategies/ETHStrategyArbV1.sol
...
contract ETHStrategyArbV1 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 constant GAS_PRICE = 10e9; // Gas price in wei, constant on BSC
    ...
    uint256 public minPercentProfit = 5; // Total profit must be greater than 0.005% for executor to earn from trade
    ...
    address constant BAKERY_ROUTER = address(0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F); // Pancake router for Ankreth
    ...
    function withdraw(address _depositor, uint256 _share, uint256 _total, bool nonContract) public onlyZSBToken returns (uint256) {
        ...
        if(nonContract == true){
            if(_share > _total.mul(percentTradeTrigger).div(DIVISION_FACTOR)){
                ...
                checkAndSwapToken(address(0), targetID);
            }
        }
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            // Sell for WBNB
            if(_idIn == 1){ //AETH
                // Must first sell via Bakeryswap for ETH then Pancakeswap for WBNB
                TradeRouter bake = TradeRouter(BAKERY_ROUTER);
                path = new address[](2);
                path[0] = address(tokenList[1].token);
                path[1] = address(tokenList[0].token);
                uint256[] memory estimates = bake.getAmountsOut(_amount, path);
                ...
                // No AETH, use only pancake
                TradeRouter cake = TradeRouter(PANCAKE_ROUTER);
                if(_idIn == 0){
                    // ETH/WBNB very liquid
                    path = new address[](2);
                    path[0] = address(tokenList[0].token);
                    path[1] = WBNB_ADDRESS;                    
                }else{
                ...
                uint256[] memory estimates = cake.getAmountsOut(_amount, path);
                _amount = estimates[estimates.length - 1]; // This is the amount of WBNB
                return _amount;
            }
        }else{
            // Sell for each other
            if(_idIn == 1 || _idOut == 1){ //AETH
                // Must first sell via Bakeryswap for ETH
                if(_idIn == 1){
                    // Selling AETH
                    TradeRouter bake = TradeRouter(BAKERY_ROUTER);
                    path = new address[](2);
                    ...
                    _amount = estimates[estimates.length - 1]; // This is the amount of ETH
                    if(_idOut == 0){
                        // We want ETH so return it
                        return _amount;
                    }
                    // Else convert to BETH from ETH
                    TradeRouter cake = TradeRouter(PANCAKE_ROUTER);
                    path = new address[](2);
                    path[0] = address(tokenList[0].token);
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            ...
            if(_idIn == 1){ //AETH
                ...
                tokenList[1].token.safeApprove(BAKERY_ROUTER, 0);
                tokenList[1].token.safeApprove(BAKERY_ROUTER, _amount);
            ...
            if(_idIn == 1 || _idOut == 1){ //AETH
                if(_idIn == 1){
                    ...
                    if(_idIn == 2){
                        ...
                        path[1] = address(tokenList[0].token);
                        _bal = tokenList[0].token.balanceOf(address(this));
                        tokenList[2].token.safeApprove(PANCAKE_ROUTER, 0);
                        tokenList[2].token.safeApprove(PANCAKE_ROUTER, _amount);
                        cake.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60));
                        _amount = tokenList[0].token.balanceOf(address(this)).sub(_bal);
                    }
                    TradeRouter bake = TradeRouter(BAKERY_ROUTER);
                    ...
                    path[0] = address(tokenList[0].token);
                    path[1] = address(tokenList[1].token);
                    tokenList[0].token.safeApprove(BAKERY_ROUTER, 0);
                    tokenList[0].token.safeApprove(BAKERY_ROUTER, _amount);
                    bake.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60));                  
                }
            }else{
                // No AETH, use only pancake
    ...
    function checkAndSwapToken(address _executor, uint256 targetID) internal {
        ...
        uint256 minGainNeeded = _totalBalance.mul(minPercentProfit).div(DIVISION_FACTOR); // Strat must product enough gain to run
        ...
        if(bnbBalance > 0){
            ...
            if(_executor != address(0)){
                uint256 gasFee = GAS_PRICE.mul(gasStipend);
    ...
    function expectedProfit(bool inWBNBForExecutor) external view returns (uint256,uint256) {
        ...
        uint256 targetID;
        uint256 minGainNeeded = balance().mul(minPercentProfit).div(DIVISION_FACTOR); // Strat must product enough gain to run
        ...
        if(_normalizedGain <= minGainNeeded){
        ...
        if(inWBNBForExecutor == false){
            ...
            uint256 gasFee = GAS_PRICE.mul(gasStipend); // This is gas stipend in wei
    ...
    function executorSwapTokens(address _executor, uint256 _minSecSinceLastTrade, uint256 _buyToken) external {
    ...
    function changeTradingConditions(uint256 _pTradeTrigger, 
                                    ...
                                    uint256 _pStipend,
                                    uint256 _maxStipend,
                                    uint256 _pMinProfit) external onlyGovernance {
        // Changes a lot of trading parameters in one call
        require(_pTradeTrigger <= 100000 && _pSellPercent <= 100000 && _pStipend <= 100000 && _pMinProfit <= 100000,"Percent cannot be greater than 100%");
