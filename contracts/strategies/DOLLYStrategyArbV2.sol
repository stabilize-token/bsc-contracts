...
// File: contracts/strategies/DOLLYStrategyArbV2.sol
...
contract DOLLYStrategyArbV2 is Ownable {
    using SafeMath for uint256;
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            if(_idIn == 1){
                router = TradeRouter(PANCAKE_ROUTER_V2);
                path = new address[](2);
                path[0] = address(tokenList[_idIn].token);
                path[1] = WBNB_ADDRESS;
            }else{
                router = TradeRouter(DOPPLE_TWINDEX);
                path = new address[](3);
                path[0] = address(tokenList[_idIn].token);
                path[1] = DOP_ADDRESS;
                path[2] = WBNB_ADDRESS;                
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        ...
        if(_idOut == WBNB_TOKEN_ID){
            if(_idIn == 1){
                ...
                path[1] = DOP_ADDRESS;
                path[2] = WBNB_ADDRESS;    
                tokenList[_idIn].token.safeApprove(DOPPLE_TWINDEX, 0);
                tokenList[_idIn].token.safeApprove(DOPPLE_TWINDEX, _amount);
            }
