...
// File: contracts/strategies/BTCStrategyArbV2.sol
...
contract BTCStrategyArbV2 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxAmountSell = 8e18; // The maximum amount of tokens that can be sold at once (normalized)
    ...
    uint256 public minAmountProfit = 1e13; // Total profit must be greater than a certain amount for executor to earn from trade
    uint256 public minTradeSplit = 1e15; // Minimum amount required before selling a percent of tokens (normalized)
    ...
    address constant VALUEPEG_DBTC_BTCB = address(0x0EF2E8a016A6874A1cC8e550Dd7Dc877210f07BD);
    address constant ELLIPSIS_REN_BTCB = address(0x2477fB288c5b4118315714ad3c7Fd7CC69b00bf9);
    ...
    function simulateExchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal view returns (uint256) {
        // renBTC is on Ellipsis with btcb
        // dbtc is on value pegswap with btcb
        CurveLikeExchange router = CurveLikeExchange(ELLIPSIS_REN_BTCB);
        ValueLikeExchange vRouter = ValueLikeExchange(VALUEPEG_DBTC_BTCB);
        if(_idIn == 1){
            // Convert renBTC to btcb using ellipsis
            _amount = router.get_dy(1, 0, _amount);
        }else if(_idIn == 2){
            // Convert dbtc to btcb using value pegswap
            _amount = vRouter.calculateSwap(0, 1, _amount);                
        }
        // Now units should be in BTCB
        if(_idOut == WBNB_TOKEN_ID){
            // Sell BTCB for WBNB
            TradeRouter pan = TradeRouter(PANCAKE_ROUTER);
            address[] memory path = new address[](2);
            path[0] = address(tokenList[0].token);
            path[1] = WBNB_ADDRESS;
            uint256[] memory estimates = pan.getAmountsOut(_amount, path);
            _amount = estimates[estimates.length - 1]; // This is the amount of WBNB returned
        }else{
            // Swap BTCB for the other token
            if(_idOut == 1){
                // Convert BTCB to renBTC
                _amount = router.get_dy(0, 1, _amount);
            }else if(_idOut == 2){
                _amount = vRouter.calculateSwap(1, 0, _amount); 
            }
        }
        return _amount;
    }
    ...
    function exchange(uint256 _idIn, uint256 _idOut, uint256 _amount) internal {
        ...
        if(_idIn == 1){
            ...
            tokenList[_idIn].token.safeApprove(ELLIPSIS_REN_BTCB, 0);
            tokenList[_idIn].token.safeApprove(ELLIPSIS_REN_BTCB, _amount);
            router.exchange(1, 0, _amount, 1);
            _amount = tokenList[0].token.balanceOf(address(this)).sub(_bal);
        }else if(_idIn == 2){
            // Convert dbtc to btcb using value pegswap
            uint256 _bal = tokenList[0].token.balanceOf(address(this));
            tokenList[_idIn].token.safeApprove(VALUEPEG_DBTC_BTCB, 0);
            tokenList[_idIn].token.safeApprove(VALUEPEG_DBTC_BTCB, _amount);
            vRouter.swap(0, 1, _amount, 1, now.add(60));
            _amount = tokenList[0].token.balanceOf(address(this)).sub(_bal);
        }
        // Now units should be in BTCB
        if(_idOut == WBNB_TOKEN_ID){
            ...
            pan.swapExactTokensForTokens(_amount, 1, path, address(this), now.add(60)); // Get WBNB from token
        }else{
            // Swap BTCB for the other token
            if(_idOut == 1){
                // Convert BTCB to renBTC
                tokenList[0].token.safeApprove(ELLIPSIS_REN_BTCB, 0);
                tokenList[0].token.safeApprove(ELLIPSIS_REN_BTCB, _amount);
                router.exchange(0, 1, _amount, 1);
            }else if(_idOut == 2){
                tokenList[0].token.safeApprove(VALUEPEG_DBTC_BTCB, 0);
                tokenList[0].token.safeApprove(VALUEPEG_DBTC_BTCB, _amount);
                vRouter.swap(1, 0, _amount, 1, now.add(60));
            }
        }
        return;
    }
    ...
    function getCheapestToken() internal view returns (uint256) {
        ...
        uint256 targetID = 0;
        uint256 mainAmount = minTradeSplit;
    ...
    function checkAndSwapToken(address _executor, uint256 targetID, bool doFlash) internal {
        ...
        bool _expectIncrease = false;
        {
            if(doFlash == false){
                ...
                // It will perform a flash loan and return the profit to this strategy as BTCB if successful
                _expectIncrease = true;
                BTCFlashModule(flashModuleAddress).doFlashTrade();
                targetID = 0;
                _allocatedGas = gasStipend.mul(3);
            }
        }
                ...
                if(stakersAmount > 0){
                    ...
                    if(zsbTokenAddress == address(0)){
                        ...
                        if(StabilizeStakingPool(zsbTokenAddress).getTotalSTBB() == 0){
                            ...
                            sendToStaking = false;
                        }else if(StabilizeStakingPool(zsbTokenAddress).getCurrentStrategy() != address(this)){
    ...
    // Param 2 is used to check for a flash loan possibility
    // Returns 1) profit, 2) uint256 tokenID to buy, 3) block number
    function expectedProfit(bool inWBNBForExecutor, bool checkFlash) external view returns (uint256,uint256,uint256) {
...

        {
            ...
            if(checkFlash == false){
                ...
                _normalizedGain = BTCFlashModule(flashModuleAddress).checkFlashTrade(); // Gain in BTBC returned (normalized)
                targetID = 0;
    ...
    // 3) uint256 blocknumber that trade will expire after 4) uint256 tokenID to buy 5) do a flash loan
